...
enum { SchemeClientNorm, SchemeClientSel, SchemeClientUrg, SchemeStatus,
       SchemeTagsSel, SchemeTagsNorm, SchemeTagsUrg, SchemeInfoSel,
...
drawbar(Monitor *m)
{
	...
	for (i = 0; i < LENGTH(tags); i++) {
		...
		unsigned int sid;
		if (urg & 1 << i)
			sid = m->tagset[m->seltags] & 1 << i ? SchemeTagsSel : SchemeTagsUrg;
		else
			sid = m->tagset[m->seltags] & 1 << i ? SchemeTagsSel : SchemeTagsNorm;
		drw_setscheme(drw, scheme[sid]);
		drw_text(drw, x, 0, w, bh, lrpad / 2, tags[i], 0, False);
		if (occ & 1 << i)
...
seturgent(Client *c, int urg)
{
	...
	XFree(wmh);
	XSetWindowBorder(dpy, c->win, scheme[SchemeClientUrg][ColBorder].pixel);
}
...
updatesystray(void)
{
	...
	if (!systray) {
		...
		if (!(systray = (Systray *)calloc(1, sizeof(Systray))))
			die("fatal: could not malloc() %u bytes\n", sizeof(Systray));
		systray->win = XCreateSimpleWindow(dpy, root, x, m->by, w, bh, 0, 0, scheme[SchemeStatus][ColBg].pixel);
